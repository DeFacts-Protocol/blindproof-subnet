...
Blindproof is a Bittensor subnet where miners run LLM inference on data they mathematically cannot read: they are paid only when a public proof confirms the work ran exactly right. 

The user client encrypts a LLM prompt, goes offline, and returns to an encrypted answer from miners who never saw the plain-text question — or the plain-text answer they themselves just computed.
